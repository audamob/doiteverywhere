...
//        Log.v(Constants.LOGTAG,targetHeight);
    ...
    public static void setUrlDrawableCustom(int width,int height,final ImageView imageView, final String url) {
        setUrlDrawableCustom(width,height,imageView.getContext(), imageView, url, null, CACHE_DURATION_THREE_DAYS, null);
    }
    private static void setUrlDrawableCustom(final int width,final int height,final Context context, final ImageView imageView, final String url, final Drawable defaultDrawable, final long cacheDurationMs, final UrlImageViewCallback callback) {
        ...
        final Loader loader = new Loader() {
            ...
            public void onDownloadComplete(UrlDownloader downloader, InputStream in, String existingFilename) {
                try {
                    ...
                    result = loadBitmapFromStreamCustom(width,height,context, url, targetFilename, targetWidth, targetHeight);
                }
...
//                        System.out.println(String.format("imageView: %dx%d, %dx%d", imageView.getMeasuredWidth(), imageView.getMeasuredHeight(), imageView.getWidth(), imageView.getHeight()));
    ...
    private static Bitmap loadBitmapFromStreamCustom(int WWidth,int HHeight,final Context context, final String url, final String filename, final int targetWidth, final int targetHeight) {
        prepareResources(context);
...
//        Log.v(Constants.LOGTAG,targetHeight);
        ...
        Log.i("ImageViewLil"," width "+WWidth);
        Log.i("ImageViewLil"," Hieght "+HHeight);
        ...
        try {
            ...
            if (mUseBitmapScaling) {
               ...
               // o.inJustDecodeBounds = true;
                ...
                BitmapFactory.decodeStream(stream, null, o);
                stream.close();
                Log.i("ImageViewLil"," width "+o.outWidth);
                Log.i("ImageViewLil"," Hieght "+o.outHeight);
                int scale = 1;
                
                
                
           /*     while ((o.outWidth >> scale) > WWidth || (o.outHeight >> scale) > HHeight) {
                    scale++;
                }*/
                o = new Options();
                o.inSampleSize = 8;
